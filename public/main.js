...
const shuffleDeck = () => {
  ...
  for (let i = 0; i < numberOptions.length; i++) {
    numberSelected = numberOptions[i];
    // console.log(numberSelected);
    for (let j = 0; j < suitOptions.length; j++) {
      suitSelected = suitOptions[j];
      // console.log(numberSelected);
      card = {
...
// //FROM LECTURE = needed to define position in array to pull
// //MAYBE USE .pop?
...
const dealButtonClick = () => {
  while (thisDeck.length > 0) {
    const position = dealCard(thisDeck.length);
    const cardToDeal = thisDeck[position];
    console.log(cardToDeal);
    thisDeckPlayedCards.push(cardToDeal);
    thisDeck.splice(position, 1);
    console.log(thisDeck);
    console.log(thisDeckPlayedCards);

    // finalCard = thisDeck[arrayItemPull];
    // console.log(thisDeckPlayedCards);
    // houseHand.hand.push(finalCard);
    ...
    const dealStartingHandToHouse = () => {
      househand.hand = [thisDeckPlayedCards.pop(), thisDeckPlayedCards.pop()];
      console.log(houseHand.hand)
  }
  ...
  for (i = 0; i <= 1; i++) {
    ...
    finalCard = thisDeck[arrayItemPull];
    playerHand.hand.push(finalCard);
  }
  document.querySelector('#playerDealtCards').textContent = playerHand.hand;
  console.log(playerHand.hand);
  // cardValuesAfterDeal();
}

// add function for manipulating the DOM
// need to break apart the hand into individual cards
// see lecture example
